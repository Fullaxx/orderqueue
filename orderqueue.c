...
#ifdef DEBUG_INSERT
			n += snprintf(&log[n], LOGSIZ-n, "... ");
			n += snprintf(&log[n], LOGSIZ-n, OQFMT, pre->num);
			n += snprintf(&log[n], LOGSIZ-n, " -> ");
#endif
			post = pre->lower;
			pre->lower = cursor;
...
#ifdef DEBUG_INSERT
	if(!pre) { n += snprintf(&log[n], LOGSIZ-n, "*** "); }
	n += snprintf(&log[n], LOGSIZ-n, OQFMT, num);
#endif
...
#ifdef DEBUG_INSERT
		n += snprintf(&log[n], LOGSIZ-n, " -> ");
		n += snprintf(&log[n], LOGSIZ-n, OQFMT, post->num);
		n += snprintf(&log[n], LOGSIZ-n, " ...");
#endif
	} else {
		/* cursor is lowest */
